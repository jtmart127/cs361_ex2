...
disasm_back_from(size_t offset, struct insn_node *node)
{
  ...
  //   rv_decode dec;
  //   decode_inst(&dec, rv64, offset-trylen, inst);

  //   if (dec.op == rv_op_illegal) {
  //     continue;
  //   }

  //   if (!is_boring(inst, offset-trylen)) {
  //     child = maybe_add_node_child(node, inst, offset-trylen);
  //     disasm_back_from(offset-trylen, child);
  //   }
  // }
  ...
  for (trylen = 1; trylen <= 8; trylen++) {
    ...
    if (dec.op == rv_op_illegal || inst_len != trylen) {
      continue;
    }
